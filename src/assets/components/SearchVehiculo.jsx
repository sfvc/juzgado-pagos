...
function SearchVehiculo ({ resetFiltro, onSelectVehiculo }) {
  ...
  const { data: vehiculo = [], isFetching, refetch } = useQuery({
  ...
  function selectVehiculo (veh) {
    setSearch(`${veh.dominio} - ${veh.titular ? deleteDuplicateName(veh.titular.apellido, veh.titular.nombre) : 'SIN TITULAR'}`)
    setShow(false)

    if (onSelectVehiculo) {
      onSelectVehiculo({ vehiculo_id: veh.id })
    }
  }
          ...
          <ul className='w-full overflow-y-auto h-max-32 absolute z-10 bg-white'>
            {isFetching
              ...
              : vehiculo.length > 0
                ? (
                    vehiculo.map((veh) => (
                      <li key={veh.id} className='hover:bg-slate-300 border-b-2 border-x px-2'><button type='button' className='mb-1 w-full text-start py-2' onClick={() => selectVehiculo(veh)}><strong>{veh.dominio}</strong> - {veh.titular ? deleteDuplicateName(veh.titular.apellido, veh.titular.nombre) : 'SIN TITULAR'}</button></li>
                    ))
                  )
                : (
                  <li className='hover:bg-slate-300 border-b-2 border-x px-2 py-2'>
